...
void printWalkTotalDurationToFile(const vector<WeeklyAgenda>& weeklyAgendas, const string& title) {
    ofstream output("C:/Users/Юыху/source/repos/Exam task2/Exam task2/report.txt");
    if (!output.is_open()) {
...
int main() {
    try {
        ...
        ifstream input("C:/Users/Юыху/source/repos/Exam task2/Exam task2/person.txt");
        if (!input.is_open()) {
            throw runtime_error("file not found");
        }
